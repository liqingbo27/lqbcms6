...
class MenuModel extends Model
{
	...
	public static function getList() {
		return self::order('pid ASC,sort ASC,id ASC')->paginate();
	}

	public static function getValueName($id){
		$info = self::find($id);
		if(!empty($info)){
			return $info->getAttr('name');
		}else{
			return '';
		}
	}
	...
	public function  getFullList($pid){
		$map = [];
		if(isset($pid)){
			$map[] = ['pid','=',$pid];
		}
		$map[] = ['status','=',1];
		$this->rawList = self::where($map)->order('sort ASC')->select()->toArray();
	...
	private function _searchList($pid = 0, $space = "") {
		...
		foreach ($childs as $key => $val) {
			...
			$childs[$key]['fullname'] = ($space ? $space . $pre : "") . $val['title'];
	...
	public static function getAllList($type='obj'){
	...
	/*  public static function getTree(){
		...
		return list_to_tree($list->toArray(),'id','pid','list');
	  }*/

	public static function getTree(){
		$objList = self::getAllList();
		return list_to_tree($objList->toArray(),'id','pid','list');
	}

	public static function updateCache(){
		$list = self::getAllList();
		$tree = list_to_tree($list);
		Cache::set('news_list',$list);
		Cache::set('news_tree',$tree);
	}
